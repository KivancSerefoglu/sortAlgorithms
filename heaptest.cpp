...
int main(){
	...
	heap.insert(8);
	heap.deleteMin();
	cout << "Testing percolate down  If there are two equal children nodes that are both smaller than the parent... " << endl;
	assert(heap.getMin() == 6);

	// ONE OF THE CHILDREN CAN BE EQUAL TO THE PARENT, WHILE THE OTHER BEING SMALLER
	heap.deleteMin();
	heap.deleteMin();
	heap.deleteMin();
	heap.deleteMin();
	cout << "Testing percolate down  If there are two children nodes, one smaller than, the other equal to the parent... " << endl;
	heap.insert(2);
	heap.insert(5);
	heap.insert(3);
	heap.insert(5);
